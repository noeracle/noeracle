...
**Date:** 2026-08-21 · **Version:** 1.0 (supersedes the v0-era page at docs.noeracle.org/concepts/threat-model)
...
The on-chain component is one Soroban contract, `oracle_v0` (~720 lines, 15 KB WASM), exposing twelve entrypoints on the live instance (verified on-chain 2026-08-21): `init`, `set_publishers`, `set_quorum`, `get_quorum`, `upgrade`, `update_batch_ed25519_args`, `update_batch_ed25519_persistent`, `update_quorum_ed25519_persistent`, `get_price`, `get_price_pers`, `prices`, `twap`. Scope details, deployed addresses and the freeze protocol are in Appendix A.
...
6. **Verify and store.** The contract checks vector lengths, the quorum gate, membership of the signing key in the admin-maintained publisher set, a 60 s staleness bound against ledger time, every per-asset signature via the host's Ed25519, and a per-asset monotonic `round_id`; only a newer round is written. Persistent writes extend the entry's TTL and append to a 32-entry per-asset history ring. On the quorum path the stored value is the per-asset **median** of the submitted publisher prices.
...
| Admin (single account) | trusted — highest residual risk | init, set_publishers, set_quorum, upgrade | write prices; bypass signature verification |
...
- **Have additional issues been found after the threat model?** Not yet; `cargo-scout-audit` and `cargo-audit` both ran clean on the frozen commit (Appendix D). The model is a living document and will be revised with the v1 multi-publisher rollout and any `upgrade()`.
...
|---|---|---|---|---|
| `oracle_v0` (`noeracle_oracle_v0`) | ✔ | Signed-price verification, storage, quorum median, history ring | 720 in `src/lib.rs` (~150 of these are feature-gated benchmark entrypoints absent from production builds) | 41 |
...
Workspace: `soroban-sdk` 26, target `wasm32v1-none`, release profile `overflow-checks = true`, `panic = "abort"`, `lto`. Production WASM 15,312 bytes.
...
**Freeze protocol.** The audited surface is the `oracle_v0` crate at the commit the live instance was built from (`c9385b1`, branch `feat/l08-l09-quorum-ring`, merged to `main`). It will be tagged; nothing lands on the tag during the engagement. Future changes ship through `upgrade()` from a tagged, hash-verified build.
...
| N-4 | No domain separation in the signed message | Distinct keys per network now; domain tag proposed for v1. Auditor input requested. |
...
- **Static analysis and advisories:** `cargo-scout-audit` 0.3.16 — 0 detections on both crates; `cargo audit` — clean (yanked-crate warnings only). Reports in `audit/`.
